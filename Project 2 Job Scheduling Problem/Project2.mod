...
param ExtremePoint { k in K_mach_RESOURCES };
...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
# Discrete machining Master Problem to find optimal solution
minimize MasterProblem:
	sum { k in K_mach_RESOURCES, j in JOBS, u in T_ALL_INTERVALS, l in 1..ExtremePoint[k] }
		c[j,u] * x_nail[j,k,u] * convexcoeff[k,l];		#Master Problem

# Discrete machining Master Problem for heuristics
minimize MasterProblemHeuristics: slackvar;

#---------------------------------------#
#Constraints for both master problems
s.t. decomposition_constraint {j in JOBS }:
	sum	{ k in K_mach_RESOURCES, u in T_ALL_INTERVALS, l in 1..ExtremePoint[k] }
		x_bar[j,k,u,l] * convexcoeff[k,l] + slackvar = 1;
		
s.t. convex_constraint { k in K_mach_RESOURCES }:
	sum { l in ExtremePoint } convexcoeff[k,l] = 1;
...
minimize SubProblemHeuristics:
	sum { j in JOBS, u in T_ALL_INTERVALS }
		(0 - pi[j])*x_nail[j,k,u] - gamma[k];
...
s.t. compatibility_constraint { J in JOBS }:
...
s.t. convex_coeff_constraint { k in K_mach_RESOURCES, l in ExtremePoint }:
	convexcoeff[k,l] >= 0;
	
s.t. out_of_bounds_constraint { j in JOBS, 
	k in K_mach_RESOURCES, u in 0..(max(r_disc[j],a_disc[j])-1) }:
		x[j,k,u] = 0;
